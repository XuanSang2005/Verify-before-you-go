...
test('local runbook documents the LAN web CORS origin required by Safari', () => {
  ...
  const documentedCors = /^CORS_ORIGINS=([^\n]+)$/mu.exec(runbook)?.[1]?.split(',') ?? [];
  const documentedAddress = documentedCors
    .map((origin) => /^http:\/\/((?:\d{1,3}\.){3}\d{1,3}):8081$/u.exec(origin)?.[1])
    .find(Boolean);
  assert.ok(documentedAddress, 'The physical-device setup must include a concrete current LAN address.');
  assert.equal(documentedCors.includes('http://localhost:8081'), true);
  assert.equal(documentedCors.includes('http://localhost:19006'), true);
